...
def api_save_word():
    ...
    try:
        ...
        logger.error(f"Save word error: {e}")
...
def api_save_phrasal():
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute(
            "INSERT INTO phrasal_verbs (verb, prepositions, russian, user_id) VALUES (%s, %s, %s, %s) RETURNING id",
        ...
        logger.error(f"Save phrasal error: {e}")
        return jsonify({'success': False, 'error': str(e)}), 500
...
def api_get_all_words():
    ...
    try:
        ...
        logger.error(f"Get words error: {e}")
        return jsonify({'words': [], 'phrasal_verbs': [], 'error': str(e)}), 500
...
def api_delete_phrasal():
    ...
    try:
        ...
        logger.error(f"Delete phrasal error: {e}")
        return jsonify({'success': False}), 500
...
def api_get_test_data():
    ...
    try:
        ...
        logger.error(f"Get test data error: {e}")
